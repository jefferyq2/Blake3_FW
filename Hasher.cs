...
namespace Cryptography.Blake3
{
    ...
    public unsafe struct Hasher : IDisposable
    {
        ...
        /// </remarks>
        public static Hash Hash(byte[] input)
        {
            var hash = new Hash();
        ...
        /// This function is always single-threaded. For multi-threading support <see cref="UpdateWithJoin"/> 
        /// </remarks>
        public static void Hash(byte[] input, byte[] output)
        {
            if (output.Length == 32)
            {
        ...
        /// A 16 KiB buffer is large enough to leverage all currently supported SIMD instruction sets.
        /// </remarks>
        public void Update<T>(byte[] data) where T : unmanaged
        {
            if (_hasher == null) ThrowNullReferenceException();
            fixed (void* ptr = data)
            {
        ...
        /// This is quite complicated compared to memory mapping.
        /// </remarks>
        public void UpdateWithJoin<T>(byte[] data) where T : unmanaged
        {
            if (_hasher == null) ThrowNullReferenceException();
            fixed (void* ptr = data)
            {
...
#pragma warning restore 465
        ...
        /// The struct returned needs to be disposed explicitly.
        /// </remarks>
        public static Hasher NewKeyed(byte[] key)
        {
        ...
        /// The struct returned needs to be disposed explicitly.
        /// </remarks>
        public static Hasher NewDeriveKey(byte[] str)
        {
            fixed(void* ptr = str)
